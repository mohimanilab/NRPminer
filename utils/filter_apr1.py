...
###################################################################
	...
	for i in range(min(thresh,len(peaks))):
		# if  peaks[sorted_peaks[i]] < minIntensity:
		# 	break
	...
	clusterPeaksCleaned = {}
	for group in autconvGroups:
		#if autconvGroups[group][0][1] < 45 or autconvGroups[group][0][1] > 200:
		#	continue
		totalSum = 0
		numElements = 0
		allPeaksGroups = []
	...
	#### GENERATING THE CLUSTER REPRESENTATIVES USING AVERAGE OF THE MEMBERS OF EACH CLUSTER
		...
		## GENERATING THE CLUSTER REPRESENTATIVES USING MEAN OF THE MEMBERS OF EACH CLUSTER
...
def output_cyclopeptide_polymers(
	...
	if numStnd >-1:
		...
		#check if the higher values of convolutions lead to "similar" convolutions (either +1\-1 like for 113 and 114 or the same 128.095 and 128.05)
		checkMasses = []
		numPolymers = []
